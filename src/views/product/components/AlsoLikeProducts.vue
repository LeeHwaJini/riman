<template>
  <section class="secondary-beige-bg love-area prd-bg-area">
    <h3>{{ $t('PR_PrdlistTitle3') }}</h3>
    <swiper
      ...
      <swiper-slide v-for="product in props.productData" :key="product.productId" class="">
        <div class="recomm-area">
          <div class="prd-img-area">
            <img :src="addPrefixForImage(product.thumbnailUrl)" alt="" @click="onClickDetail(product.productId, 'get-product-detail')">
          ...
          <div class="prd-list-info-area">
            ...
            <p class="prd-detail-text" @click="onClickDetail(product.productId, 'get-product-detail')">{{ product.productName }}</p>
            ...
            <ul class="prd-point-area">
              <template v-if="isLogined && isPlanner">
                <li class="sp">
                  <span>{{ product.spOriginal }}</span><span>SP</span>
                </li>
                <li class="bp">
                  <span>{{ product.bpOriginal }}</span><span>BP</span>
                </li>
              </template>

              <li class="p">
                <span>{{ product.point }}</span><span>P</span>
              </li>
            </ul>
            <AddCartDrawerButton
              :product="product"
    ...
    <div class="text-img-area">
      <img src="@assets/images/love_text.svg" alt="">
    </div>
  </section>
</template>
